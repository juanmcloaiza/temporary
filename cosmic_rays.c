...
void CR_test_routine(void)
{
  ...
  if(ThisTask == 0)
    {
      ...
      for(SphP[i].a2.Density = 1.01 * All.PhysDensThresh; SphP[i].a2.Density <= 10000.0 * All.PhysDensThresh;
	  SphP[i].a2.Density *= 1.05)
	{
	  tsfr = sqrt(All.PhysDensThresh / (SphP[i].a2.Density * a3inv)) * All.MaxSfrTimescale;

	  factorEVP = pow(SphP[i].a2.Density * a3inv / All.PhysDensThresh, -0.8) * All.FactorEVP;
	  ...
	  egy_cr = (p * All.FeedbackEnergy * All.CR_SNEff / dtime) * fraction
	    * CR_Tab_GetCoolingTimescale(q, SphP[i].a2.Density);

	  bet = CR_Tab_Beta(q);

	  P_cr =
	    bet / 6.0 * (All.CR_Alpha - 1) * SphP[i].a2.Density * egy_cr / (0.5 * bet +
									    pow(q,
	  ...
	  fprintf(fd, "%g  %g %g %g %g %g    %g   %g %g  %g %g  %g\n",
		  SphP[i].a2.Density, tsfr, tcool, th, trelax, tdyn, egyeff, qinj, q, egy_cr, P_cr, fraction);
	}
      fclose(fd);

      SphP[i].a2.Density = 1.01 * All.PhysDensThresh;
      ...
      for(q = 0.001; q <= 1000.0; q *= 1.025)
	{
	  ...
	  fprintf(fd, "%g  %g %g %g  %g %g  %g %g\n", q,
		  CR_Particle_GetThermalizationTimescale(q, SphP[i].a2.Density),
		  CR_Particle_GetDissipationTimescale(q, SphP[i].a2.Density),
		  CR_Particle_GetCoulombTimescale(q, SphP[i].a2.Density), egy, egy_sn,
		  CR_Particle_GetCoolingTimescale(q, SphP[i].a2.Density),
  ...
  if(ThisTask == 0)
	...
	egycurrent = SphP[i].Entropy * pow(SphP[i].a2.Density * a3inv, GAMMA_MINUS1) / GAMMA_MINUS1;
	...
	SphP[i].Entropy =
	  (egyeff +
	   (egycurrent -
	    egyeff) * exp(-dtime / trelax)) * GAMMA_MINUS1 / pow(SphP[i].a2.Density * a3inv, GAMMA_MINUS1);

	cloudmass = x * P[i].Mass;

	tcr =
	  CR_Particle_GetCoolingTimescale(SphP[i].CR_q0 * pow(SphP[i].a2.Density, 0.3333),
					  SphP[i].a2.Density);
	...
	coolrate = SphP[i].CR_E0 / CR_Tab_GetCoolingTimescale(SphP[i].CR_q0 * pow(SphP[i].a2.Density, 0.3333),
							      SphP[i].a2.Density);
	...
	printf
	  ("step=%d dt=%g egycurrent=%g(temp=%g)  CR_e0=%g(temp=%g) sn_raw=%g  cr_heat=%g  cr_cool=%g (%g)   Q0=%g \n",
	   ...
	   SphP[i].CR_q0 * pow(SphP[i].a2.Density, 0.3333));
