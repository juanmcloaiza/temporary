...
//Added by JM
     //Update the list of particle types: 
      for(i = 0; i < 6 ; i++) NtypeLocal[i] = 0 ;
      for(i = 0; i < NumPart ; i++) NtypeLocal[P[i].Type]++ ;
      temp = malloc(NTask * 6 * sizeof(int));
      MPI_Allgather(NtypeLocal, 6, MPI_INT, temp, 6, MPI_INT, MPI_COMM_WORLD);
      for(i = 0; i < 6; i++){
        Ntype[i] = 0;
        for(j = 0; j < NTask; j++) Ntype[i] += temp[j * 6 + i];
      }
      free(temp);
     //Start the accretion flagging for dom. dec. if necessary 
      All.dom_dec_flag = 0;
      accrete_particles();
      temp = malloc(NTask * sizeof(int)); 

      MPI_Allgather(&flag_accretion_threshold, 1, MPI_INT, temp, 1, MPI_INT, MPI_COMM_WORLD); 
      for(i = 0; i< NTask; i++) { 
        All.dom_dec_flag += temp[i]; 
      } 
      free(temp);
      if(All.dom_dec_flag > 0)
...
void every_timestep_stuff(void)
{
  ...
  if(ThisTask == 0)
    {
      //Added by JM - Por qué chingados no pusieron esta madre en el Gadget original??!!
//      if(All.Time > 0 && All.TimeStep <= 0.0){
  //        printf("\n\nTimestep menor o igual a cero!!!!!\n\n");
    //      exit(EXIT_FAILURE);
    //  }
      //End of Added by JM
      if(All.ComovingIntegrationOn)
      ...
      else
    {
      ...
      printf("Number of particles: %d, Gas particles: %d\n", All.TotNumPart, All.TotN_gas);
      fflush(FdInfo);
    }
