      ...
      if(P[n].Type == 0)
	{
	  ...
	  if(r2 < PPP[n].Hsml * PPP[n].Hsml)
		...
		particles[count_local].Pos[k] = P[n].Pos[k];
	      ...
	      particles[count_local].Utherm = SphP[n].Entropy / GAMMA_MINUS1 * pow(SphP[n].a2.Density *
										   a3inv, GAMMA_MINUS1);
		  ...
		  if(r < PPP[n].Hsml)
			...
			bin += PIXELS;
		      ...
		      utherm = DMAX(All.MinEgySpec,
				    SphP[n].Entropy / GAMMA_MINUS1 * pow(SphP[n].a2.Density *
									 a3inv, GAMMA_MINUS1));

		      AbundanceRatios(utherm, SphP[n].a2.Density * a3inv, &ne, &nh0, &nHeII);
