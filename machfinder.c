...
void GetMachNumber(struct sph_particle_data *Particle)
{
  ...
  csnd = sqrt(GAMMA * Particle->Pressure / Particle->a2.Density) * pow(atime, -3. / 2. * GAMMA_MINUS1);
...
void GetShock_DtEnergy(struct sph_particle_data *Particle)
{
  ...
  if(All.ComovingIntegrationOn)
	...
	pow(Particle->a2.Density, GAMMA_MINUS1) * pow(All.Time, -3.0 * GAMMA_MINUS1);
    }
  else
    {
      dudt = Particle->e.DtEntropy * pow(Particle->a2.Density, GAMMA_MINUS1) / GAMMA_MINUS1;
    }
