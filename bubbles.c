...
void bubble(void)
{
  ...
  if(All.ComovingIntegrationOn)
	...
	pow(All.BiggestGroupMass / 1.0e5, 5. / 3.) * pow(hubble_a, 2. / 3.);
#else
  ...
  do
      ...
      for(n = 0; n < numngb_inbox; n++)
	{
	  ...
	  if(r2 < BubbleRadius * BubbleRadius)
		...
		E_bubble += SphP[j].Entropy * P[j].Mass * pow(SphP[j].a2.Density, GAMMA_MINUS1) / GAMMA_MINUS1;
      ...
      for(n = 0; n < numngb_inbox; n++)
	{
	  ...
	  if(r2 < BubbleRadius * BubbleRadius)
	      ...
	      if(dE == 0)
		{
	      ...
	      if(All.ComovingIntegrationOn)
		SphP[j].Entropy +=
		  GAMMA_MINUS1 * dE / P[j].Mass / pow(SphP[j].a2.Density / pow(All.Time, 3), GAMMA_MINUS1);
	      else
		SphP[j].Entropy += GAMMA_MINUS1 * dE / P[j].Mass / pow(SphP[j].a2.Density, GAMMA_MINUS1);
